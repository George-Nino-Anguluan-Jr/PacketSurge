...
var _debug_section_node: VBoxContainer = null
var _version_tap_count: int = 0
var _version_tap_timer: float = 0.0
const _TAP_THRESHOLD: int = 5
...
func _build_general() -> void:
	...
	general_content.add_child(_make_tappable_version_row("Version", "v0.1.0", label_font, value_font, card_pad))
	...
	# DEBUG section — hidden until version tapped 5 times
	_debug_section_node = VBoxContainer.new()
	_debug_section_node.add_theme_constant_override("separation", 16)
	_debug_section_node.visible = GameManager.settings_debug_unlocked
	general_content.add_child(_debug_section_node)
	_debug_section_node.add_child(_make_divider())
	_debug_section_node.add_child(_make_section_label("DEBUG", section_font))
	_debug_section_node.add_child(_make_command_panel_button(label_font, card_pad, row_sep))
...
# ─── TAPPABLE VERSION ROW ──────────────────────────────
func _make_tappable_version_row(label: String, value: String, label_font: int, value_font: int, card_pad: int) -> PanelContainer:
	var card := PanelContainer.new()
	card.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	...
	card.gui_input.connect(_on_version_tapped)
...
func _on_version_tapped(event: InputEvent) -> void:
	if not (event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT):
		return
	var now := Time.get_ticks_msec() / 1000.0
	if now - _version_tap_timer > 2.0:
		_version_tap_count = 0
	_version_tap_timer = now
	_version_tap_count += 1

	if _version_tap_count >= _TAP_THRESHOLD and not GameManager.settings_debug_unlocked:
		GameManager.settings_debug_unlocked = true
		_debug_section_node.visible = true
...
func _on_toggle_pressed(toggle: Button, on_toggle: Callable) -> void:
	...
	_style_toggle_button(toggle, new_val, _fs(0.045, 16.0, 20.0))
