...
func _make_toggle_row(
	...
	var lbl := Label.new()
	lbl.text = label
	lbl.add_theme_font_size_override("font_size", 18 if _is_compact() else 17)
	...
	desc.add_theme_font_size_override("font_size", 14 if _is_compact() else 13)
	desc.add_theme_color_override("font_color", Color("#4A7FA5"))
	text_col.add_child(desc)
	row.add_child(text_col)

	var toggle := Button.new()
	toggle.custom_minimum_size = Vector2(90 if _is_compact() else 90, 48 if _is_compact() else 40)
...
func _make_slider_row(
	...
	slider.custom_minimum_size = Vector2(90 if _is_compact() else 100, 0)
	...
	pct_label.custom_minimum_size = Vector2(44 if _is_compact() else 44, 0)
	pct_label.text = str(int(current_value * 100)) + "%"
	pct_label.add_theme_font_size_override("font_size", 18 if _is_compact() else 17)
...
func _make_section_label(text: String) -> Label:
	var lbl := Label.new()
	lbl.text = text
	lbl.add_theme_font_size_override("font_size", 16 if _is_compact() else 15)
...
func _make_info_row(label: String, value: String) -> PanelContainer:
	...
	var lbl := Label.new()
	lbl.text = label
	lbl.custom_minimum_size = Vector2(120 if _is_compact() else 130, 0)
	lbl.add_theme_font_size_override("font_size", 17 if _is_compact() else 16)
	...
	val.add_theme_font_size_override("font_size", 17 if _is_compact() else 16)
...
func _apply_styles() -> void:
	...
	back_btn.add_theme_font_size_override("font_size", 18 if _is_compact() else 17)
...
func _style_toggle_button(btn: Button, is_on: bool) -> void:
	...
	btn.add_theme_font_size_override("font_size", 16 if _is_compact() else 15)
