...
var step_codes = [
	"arr = [5,3,8,1,2] → key=3, insert before 5",
	"[3,5,8,1,2] → key=8, already in place",
	"[3,5,8,1,2] → key=1, insert at start",
	"[1,3,5,8,2] → key=2, insert after 1",
	"[1,2,3,5,8] ✅ Sorted!",
]

var key_indices = [1, 2, 3, 4, -1]
...
func _build_ui() -> void:
	...
	diag_title.text = "📊 Insertion Sort — Insert Each Element"
	...
	code_label.add_theme_color_override("font_color", COL_KEY)
...
func _draw_diagram() -> void:
	...
	diagram_area.draw_string(font, Vector2(16, 20), "def insertion_sort(arr):", HORIZONTAL_ALIGNMENT_LEFT, -1, 13, COL_LABEL)
	diagram_area.draw_string(font, Vector2(16, 40), "    for i in range(1, len(arr)):", HORIZONTAL_ALIGNMENT_LEFT, -1, 13, COL_TEXT)
	diagram_area.draw_string(font, Vector2(16, 60), "        key = arr[i]", HORIZONTAL_ALIGNMENT_LEFT, -1, 13, COL_KEY)
	diagram_area.draw_string(font, Vector2(16, 80), "        insert key into correct position", HORIZONTAL_ALIGNMENT_LEFT, -1, 13, Color("#FF3366"))
...
func _draw_animation() -> void:
	...
	var ki   = key_indices[idx] if idx < key_indices.size() else -1
	var bw   = 48.0; var bh = 48.0
	var total = arr.size() * (bw + 6) - 6
	var sx    = (w - total) / 2.0
	var y     = 16.0
	var is_done = (idx == arr_states.size() - 1)
	...
	for i in range(arr.size()):
		...
		if i == ki and not is_done:
			anim_area.draw_string(font, Vector2(x + 8, y + bh + 10), "KEY", HORIZONTAL_ALIGNMENT_LEFT, -1, 10, COL_KEY)
